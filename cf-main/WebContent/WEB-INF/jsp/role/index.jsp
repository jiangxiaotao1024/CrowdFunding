...
<link rel="stylesheet"
	href="${APP_PATH }/jquery/pagination_zh/lib/pagination.css">
<style>
.tree li {
...
<body>
							...
							<div class="form-group has-feedback">
								<div class="input-group">
									...
									<input class="form-control has-success" type="text"
										placeholder="请输入查询条件" id="word">
								</div>
							</div>
							<button type="button" class="btn btn-warning" onclick="querypage(1)">
								<i class="glyphicon glyphicon-search"></i> 查询
							</button>
						</form>
						<button type="button" class="btn btn-danger"
							style="float: right; margin-left: 10px;" id="deleteByBatch">
	...
	<script
		src="${APP_PATH }/jquery/pagination_zh/lib/jquery.pagination.js"></script>
	<script type="text/javascript">
								...
								$.each(data,
										function(i, n) {
											...
											context += '<td>' + n.name
													+ '</td>';
											context += '<td>';
											context += '<button type="button" class="btn btn-success btn-xs" onclick="assignPermission('+n.id+')"><i class=" glyphicon glyphicon-check"></i></button>';
		...
		function assignPermission(id) {
			window.location.href="${APP_PATH}/role/assignPermission.htm?id="+id;
		}
		function update(n) {
			window.location.href="${APP_PATH}/role/update.htm?id="+n;
		}
		...
		$("#deleteByBatch").click(function(){
			...
			$.ajax({
				type:"POST",
				url:"${APP_PATH}/role/deleteByBatch.do",
				data:jsonObj,
				success:function(result){
					if(result.success){
						querypage(1);
